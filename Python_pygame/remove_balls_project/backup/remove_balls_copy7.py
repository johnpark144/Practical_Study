def final() :
    ...
    ball_images = [
        pygame.image.load(os.path.join(image_path, "balloon0-2.png")),
        pygame.image.load(os.path.join(image_path, "balloon1-2.png")),
        pygame.image.load(os.path.join(image_path, "balloon2-2.png")),
        pygame.image.load(os.path.join(image_path, "balloon3-2.png")),
        pygame.image.load(os.path.join(image_path, "balloon4-2.png"))]


    # 공 크기에 따른 최초 스피드
    ball_speed_y = [-19, -16, -13, -10, -7] # index 0, 1, 2, 3에 해당하는 값
    ...
    balls.append({
        ...
        "to_x" : 4, # x축 이동방향, -3 이면 왼쪽으로, 3이면 오른쪽으로
        "to_y" : -6,# y축 이동방향,
    ...
    msg = game_font.render("Final", True, (255, 255, 0)) # 노란색
    ...
    while running :
        ...
        for ball_idx, ball_val in enumerate(balls):
            ...
            for weapon_idx, weapon_val in enumerate(weapons):
                ...
                continue # 안쪽 for 문 조건이 맞지 않으면 continue, 바깥 for문 계속수행
            break # 안쪽 for 문에서 break 만나면 여기로 진입가능. 2중 for문 한번에 탈출
