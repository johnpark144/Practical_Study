...
// gitignore
// 과거로 되돌리기 -- Reset, Revert, Restore
// 브랜치 확인, 생성, 수정, 삭제, 이동
// 브랜치 합치기 -- Merge, --squash, Rebase, 충돌해결
...
git difftool (커밋아이디1) (커밋아이디2)  // 두 특정 커밋끼리 다른내용 서로 비교
...
// Restore : 파일 하나만 과거로 되돌림

git log	// comit 로그 확인하여 돌아갈 과거 일련번호 확인 (Vim)	// 소스트리로 쉽게 확인가능
...
// ######## Restore
git restore (파일명)	// 현재 파일을 최근 commit 된 상태로 수정내역을 되돌림
git restore --source (일련번호 앞 일부) (파일명)	// 파일을 특정 커밋아이디 시점으로 복구
...
// ######## Merge --squash
git merge --squash (합칠 브랜치명) // 브랜치에 있는 코밋들을 다합쳐서 하나의 코밋으로 Main에 남겨줌 
...
// 원격에서 다운 받은경우 항상 .gitignore랑 .env 업데이트 해줄것
