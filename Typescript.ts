...
// 타입스크립트 버그회피
// 여러 타입 -- 숫자로된 배열만, number[], Array<number>, any보다 권장, unknown, 튜플타입, 함수에서 아무것도, void, 에러를 반환, 영원히 끝나지 않는 함수, never 
// enum 
// 사용자 타입지정 -- interface, type 
// interface와 type의 차이점 -- 확장 방법 
// implements, extends --  implements는 클래스의 타입을 interface로 정한 타입으로, extends는 기존에 있는 interface로
// 함수 -- 함수 오버로드
// 유니온 타입 인터섹션 타입 -- 교차 타입
// 클래스 접근 제한자 -- public, private, protected, readonly, static, abstract class, 상속용 class, 
// 제네릭 -- 타입을 미리 정하지 않고 사용할때 결정
// 유틸리티 타입 -- key들만 타입으로 사용, keyof , 부분적으로만 사용, Partial<T>, 파트도 무조건 사용, Required<T>, Readonly<T>
// , (key값의 타입, value의 타입 따로지정), Record<K, T>, Pick<T, K>, Omit<T, K>, Exclude<T1, T2>, NonNullable<T>
